...
model {
  ...
  for(i in 1:N)
    y[i] = (beta[1, pixel[i]]) +
        ((beta[2, pixel[i]] - (doy[i] * beta[4, pixel[i]])) / 
        (1 + exp(beta[3, pixel[i]] * (doy[i] - (gamma[pixel[i]] + phi[year[i]])))));
...
generated quantities {
  ...
  for (n in 1:N) 
    sim[n] = normal_rng((beta[1, pixel[n]]) +
        ((beta[2, pixel[n]] - (doy[n] * beta[4, pixel[n]])) /
        (1 + exp(beta[3, pixel[n]] * (doy[n] - (gamma[pixel[n]] + phi[year[n]]))))), sigma);
